...
// repo copy).
//
// The project owns its specs — they accumulate real project lessons, so a hand-edited file is
// NEVER overwritten. But "never overwrite" alone means an upstream fix (a leaked issue key, a
// wrong instruction) can never reach a project that already onboarded, which is how a shipped
// bug outlives its fix indefinitely.
//
// So: refresh a spec only when it is byte-identical to some version this stack has shipped
// (loops/.known-hashes.json, generated by scripts/gen-spec-hashes.mjs). That proves nobody
// edited it, which makes replacing it lossless. Anything else is a local edit — left untouched
// and reported, so the user can merge the upstream change themselves.
const specSrcDir = path.resolve(SCRIPT_DIR, "..", "..", "loops");
const shaOf = (buf) => crypto.createHash("sha256").update(buf).digest("hex");
const knownHashes = (readJSON(path.resolve(specSrcDir, ".known-hashes.json")) || {}).files || {};
const refreshed = [];
const customized = [];
try {
  for (const f of fs.readdirSync(specSrcDir).filter((n) => n.endsWith(".md"))) {
    const src = path.resolve(specSrcDir, f);
    ...
    if (shaOf(current) === shaOf(shipped)) continue;             // already current
    if ((knownHashes[f] || []).includes(shaOf(current))) {
      fs.writeFileSync(dst, shipped);
      refreshed.push(path.relative(ROOT, dst));
    } else {
      customized.push(path.relative(ROOT, dst));
    }
  }
} catch {
...
if (refreshed.length) {
  console.log("  Refreshed " + refreshed.length + " unmodified loop spec(s) to the current version: " + refreshed.join(", ") + ".");
}
if (customized.length) {
  console.log("  Kept your edited loop spec(s) as-is (they differ from every shipped version): " + customized.join(", ") + ".");
  console.log("    These do NOT receive upstream fixes. Diff them against the stack's loops/ if you want the latest wording.");
}
...
// Portability: harnesses other than Claude Code read AGENTS.md. Drop a pointer, not a copy —
// two full copies drift the moment someone edits one, and a stale second copy is worse than
// none because it looks authoritative. CLAUDE.md stays the single source of truth (Claude Code
// loads it automatically); AGENTS.md just routes every other agent to it.
const agentsMdPath = path.resolve(ROOT, "AGENTS.md");
if (!fs.existsSync(agentsMdPath)) {
  const pointer = [
    "# AGENTS.md",
    "",
    "**Read [`CLAUDE.md`](./CLAUDE.md) in full before doing anything.** It is the single source of",
    "truth for this repo's instructions — invariants, response format, and the skills routing map.",
    "This file exists only so agents that look for `AGENTS.md` find it; the content is deliberately",
    "not duplicated here, because two copies drift and a stale copy reads as authoritative.",
    "",
    "All project specifics (issue tracker, branch model, package manager, frameworks, backend/DB,",
    "tests, CI/deploy, design tool) live in [`.claude/stack.md`](./.claude/stack.md), generated by",
    "the loop-stack `onboard` skill. Use its values; never assume a specific tool. If a capability",
    "is `none`, skip those steps — don't ask, don't invent.",
    "",
  ].join("\n");
  try {
    fs.writeFileSync(agentsMdPath, pointer);
    console.log("  Wrote " + path.relative(ROOT, agentsMdPath) + " (pointer to CLAUDE.md for non-Claude-Code agents).");
  } catch {
    console.log("  Note: add an AGENTS.md pointing at CLAUDE.md so non-Claude-Code agents pick up the same instructions.");
  }
} else {
  console.log("  AGENTS.md already exists — left untouched. Make sure it points at CLAUDE.md.");
}
...
const scanned = [path.resolve(OUT, "stack.md"), claudeMdPath, agentsMdPath, ...(() => {
